			...
			<view class="device_info" >
				<view>
					<device_List_copd :devices="COPDDeviceInfos" :isConnected="isConnectedCOPD" :showCheckModal="showCheckModalCOPD" 
						...
						:userId="userId" :isneedECG="needECG"
						@start-detection="startCOPDDetection"
						@end-detection="endCOPDDetection"
						@end-window="confirmExitCheck"
...
<script>
	...
	import {
	    appendCOPDDataToFile, uploadCopdFilesToOss,
           ...
           uni.showModal({
				...
				uni.showModal({
					...
					success: async function(res) {
						...
						uni.setStorage({
							...
							//上传睡眠数据
						let isSuccess = await uploadCopdFilesToOss(that.startCOPDTimeTamp, endTimeTamp)
							// 文件上传有误
						if (!isSuccess) {
							console.log("上传错误")
						}else{
			...
			noticeBluetoothStartSent(id) {
				...
				if (that.isConnectedSleep) {
					...
					console.log('copd')
					setTimeout(()=>{uni.writeBLECharacteristicValue({
						deviceId: that.connectBlueDeviceId,
						serviceId: that.BLEInformation.writeServiceId,
						characteristicId: that.BLEInformation.writeCharaterId,
						value: dataView,
